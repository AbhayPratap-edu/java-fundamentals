package Lafor_Arrays;

public class MSTree {
    public static void main(String[] args){
        GraphMST theGraph = new GraphMST();
        theGraph.addVertex('A'); // 0 (start for mst)
        theGraph.addVertex('B'); // 1
        theGraph.addVertex('C'); // 2
        theGraph.addVertex('D'); // 3
        theGraph.addVertex('E'); // 3

        theGraph.addEdge(0, 1); // AB
        theGraph.addEdge(0, 2); // AC
        theGraph.addEdge(0, 3); // AD
        theGraph.addEdge(0, 4); // AE
        theGraph.addEdge(1, 2); // BC
        theGraph.addEdge(1, 3); // BD
        theGraph.addEdge(1, 4); // BE
        theGraph.addEdge(2, 3); // CD
        theGraph.addEdge(2, 4); // CE
        theGraph.addEdge(3, 4); // DE

        System.out.print("Minimum spanning tree: ");
        theGraph.mst();
        System.out.println();
    }
}

class GraphMST{
    private final int MAX_VERTS = 20;
    private VertexMS[] vertexList; // list of vertices
    private int adjMat[][];
    private int nVerts;
    private StackMS theStack;

    public GraphMST(){
        vertexList = new VertexMS[MAX_VERTS];
        adjMat = new int[MAX_VERTS][MAX_VERTS];
        nVerts = 0;
        for(int j=0; j<MAX_VERTS; j++)
            for(int k=0; k<MAX_VERTS; k++)
                adjMat[j][k] = 0;
        theStack = new StackMS();
    }

    public void addVertex(char lab){
        vertexList[nVerts++] =  new VertexMS(lab);
    }

    public void addEdge(int start,int end){
        adjMat[start][end] = 1;
        adjMat[end][start] = 1;
    }

    public void displayVertex(int v){
        System.out.print(vertexList[v].label);
    }

    public void mst() // minimum spanning tree (depth first)
    {
        vertexList[0].wasVisited = true; // mark it
        theStack.push(0); // push it

        while ( !theStack.isEmpty() ) // until stack empty get stack top
        {
            int currentVertex = theStack.peek();
            // get next unvisited neighbor
            int v = getAdjUnvisitedVertex(currentVertex);
            if( v == -1) // if no more neighbors
                theStack.pop(); // pop it away
            else {
                vertexList[v].wasVisited = true;
                theStack.push(v);
                // display edge
                displayVertex(currentVertex); // from currentV
                displayVertex(v); // to v
                System.out.print(" ");
            }
        } // end while stack is not empty

        for(int j=0; j<nVerts; j++) // reset flags
            vertexList[j].wasVisited = false;
    } // end msTree

    private int getAdjUnvisitedVertex(int v) {
        for(int j=0; j<nVerts; j++)
            if(adjMat[v][j] == 1 && !vertexList[j].wasVisited)
                return j;
        return -1;
    }
}

class VertexMS{
    public char label;
    public boolean wasVisited;

    public VertexMS(char lab){
        label = lab;
        wasVisited =false;
    }
}

class StackMS{
    private final int SIZE = 20;
    private int[] stack;
    private int top;

    public StackMS(){
        stack = new int[SIZE];
        top = -1;
    }

    public void push(int j){
        stack[++top] = j;
    }

    public int pop(){
        return stack[top--];
    }

    public int peek(){
        return stack[top];
    }

    public boolean isEmpty(){
        return top == -1;
    }
}
